...
class NF(MeanField):
    def __init__(self, shape, use_bias, t=1, train_initial=True):
        super().__init__(shape, use_bias)
        self.jac_w = 0.
        self.jac_b = 0.
        ...
        self.weight_mu.requires_grad_(train_initial)
        self.weight_rho.requires_grad_(train_initial)
        self.bias_mu.requires_grad_(train_initial)
        self.bias_rho.requires_grad_(train_initial)
    ...
    def get_weight_sample(self):
        ...
        for t in self.transform_w:
            w = t(w_0)
            self.jac_w += t.log_abs_det_jacobian(w_0, w)[0]
            w_0 = w
    ...
    def get_bias_sample(self):
        ...
        for t in self.transform_b:
            b = t(b_0)
            self.jac_b += t.log_abs_det_jacobian(b_0, b)[0]
            b_0 = b
        return b[0]
...
class IAFMixin:
    def get_transform_w(self):
        ...
        transform_w = nn.ModuleList(
            [AffineAutoregressive(AutoRegressiveNN(prod, [prod]), stable=True) for _ in range(self.t)])
        return transform_w

    def get_transform_b(self):
        transform_b = nn.ModuleList(
            [AffineAutoregressive(AutoRegressiveNN(self.shape[0], [self.shape[0]]), stable=True) for _ in
             range(self.t)])
        return transform_b
